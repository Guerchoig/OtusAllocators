...
public:
    ...
    ~my_vector()
    {
        std::allocator_traits<Allocator>::deallocate(alloc, data, 0);
    }
...
my_vector<T, Allocator>::my_vector(const my_vector<T, Allocator> &another)
{
    ...
    data = std::allocator_traits<Allocator>::allocate(alloc, first_free_idx);
    std::memcpy(data, another.data, another.first_free_idx);
}

template <class T, class Allocator>
my_vector<T, Allocator> &my_vector<T, Allocator>::operator=(const my_vector<T, Allocator> &another)
{
    my_vector<T, Allocator>::my_vector(another);
    return this;
}
...
template <class T, class Allocator>
inline void my_vector<T, Allocator>::reserve(const std::size_t capacity)
{
    if (capacity > first_free_idx)
    {
        // Reserves memory of size capacity for data
        T *temp = std::allocator_traits<T>::allocate(alloc, capacity);
